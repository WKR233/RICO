...
def load_rgb(path, normalize_rgb = False):
    ...
    if normalize_rgb: # [-1,1] --> [0,1]
        img -= 0.5
        img *= 2.
    img = img.transpose(2, 0, 1) # (height, width, channels) 转换为 (channels, height, width)
    return img


def load_K_Rt_from_P(filename, P=None): # P是投影矩阵（4*3）
    if P is None:
    ...
    K = K/K[2,2] #归一化（让（2，2）变为1）
    intrinsics = np.eye(4)
    intrinsics[:3, :3] = K #齐次化

    pose = np.eye(4, dtype=np.float32)
    pose[:3, :3] = R.transpose()
    pose[:3,3] = (t[:3] / t[3])[:,0] #利用R和t构建pose((R, t),(t^t, 1))
...
def get_camera_params(uv, pose, intrinsics):
    ...
    return ray_dirs, cam_loc #射线方向&相机位置
...
def lift(x, y, z, intrinsics): # 计算相机坐标系下的三维坐标
    # parse intrinsics
    intrinsics = intrinsics.cuda()
    fx = intrinsics[:, 0, 0] #焦距
    fy = intrinsics[:, 1, 1]
    cx = intrinsics[:, 0, 2] #主点
    cy = intrinsics[:, 1, 2]
...
def get_sphere_intersections(cam_loc, ray_directions, r = 1.0):
    ...
    sphere_intersections = sphere_intersections.clamp_min(0.0) #将张量中的元素限制在0.0以上

    return sphere_intersections # 返回一个（n_rays， 2）的张量，代表交点离每条射线的原点的距离
