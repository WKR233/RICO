...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_mesh, plot_nimgs, resolution, grid_boundary,  level=0):
    ...
    if plot_mesh:
        surface_traces = get_surface_sliding(send_path=path,
                                            epoch=epoch,
...
def plot_rico(implicit_network, indices, plot_data, path, epoch, img_res, plot_mesh, plot_nimgs, resolution, grid_boundary, level=0):
    ...
    if plot_data is not None:
        ...
        # plot semantic maps
        plot_seg_images(plot_data['semantic_map'], plot_data['semantic_gt'], path, epoch, plot_nimgs, img_res, indices)

        # concat output images to single large image
        images = []
        for name in ["rendering", "semantic", "depth", "normal"]:
            images.append(cv2.imread('{0}/{1}_{2}_{3}.png'.format(path, name, epoch, indices[0])))        
    ...
    if plot_mesh:
        ...
        for indx in range(sem_num):
            # plot each object and background and save in different files
            _ = get_surface_sliding(
                send_path=[path, str(indx)],
                epoch=epoch,
        ...
        surface_traces = get_surface_sliding(
            send_path=[path, 'all'],
            epoch=epoch,
            sdf=lambda x: -f(-implicit_network(x)[:, :sem_num].unsqueeze(1)).squeeze(-1).squeeze(-1),
            resolution=resolution,
            grid_boundary=grid_boundary,
            level=level
        )
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                ...
                x = np.linspace(x_min, x_max, cropN) # 进一步分割
                y = np.linspace(y_min, y_max, cropN)
                z = np.linspace(z_min, z_max, cropN)

                xx, yy, zz = np.meshgrid(x, y, z, indexing='ij')
                points = torch.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=torch.float).cuda() # 每个分块的点集合（3，cropN，cropN，cropN）
                ...
                # construct point pyramids 即多次池化，得到不断模糊与规模不断变小的数据
                ...
                # evalute pyramid with mask 逐层评估sdf
                mask = None
                threshold = 2 * (x_max - x_min)/cropN * 8
                for pid, pts in enumerate(points_pyramid): # pid代表层数
                    coarse_N = pts.shape[-1]
                    pts = pts.reshape(3, -1).permute(1, 0).contiguous() # contiguous用于保证张量占用的内存空间连续
                    
                    if mask is None:    
                        pts_sdf = evaluate(pts)
                    else:                    
                        mask = mask.reshape(-1)
                        pts_to_eval = pts[mask] # 根据mask继续选择评估的点
                ...
                # 提取等值面&创建3d表示
                if (not (np.min(z) > level or np.max(z) < level)): #不需要考虑不存在表面的地方（sdf恒>0或者sdf恒<0）
...
def get_surface_by_grid(grid_params, sdf, resolution=100, level=0, higher_res=False):
    ...
    if higher_res:
        ...
        verts, faces, normals, values = measure.marching_cubes(
            ...
            spacing=(grid['xyz'][0][2] - grid['xyz'][0][1],
                     grid['xyz'][0][2] - grid['xyz'][0][1],
                     grid['xyz'][0][2] - grid['xyz'][0][1])) #使用marching_cubes算法

        verts = verts + np.array([grid['xyz'][0][0], grid['xyz'][1][0], grid['xyz'][2][0]]) #调整顶点位置（匹配）

        mesh_low_res = trimesh.Trimesh(verts, faces, normals) # 三维模型
        components = mesh_low_res.split(only_watertight=False)
        areas = np.array([c.area for c in components], dtype=np.float)
        mesh_low_res = components[areas.argmax()]
...
def get_grid_uniform(resolution, grid_boundary=[-2.0, 2.0]): #根据boundary生成均匀的三维网格
...
def get_grid(points, resolution, input_min=None, input_max=None, eps=0.1): #生成一个三维网格，均匀分布的网格点
    if input_min is None or input_max is None:
        input_min = torch.min(points, dim=0)[0].squeeze().numpy()
        input_max = torch.max(points, dim=0)[0].squeeze().numpy() #points仅仅用来界定网格的范围

    bounding_box = input_max - input_min
    shortest_axis = np.argmin(bounding_box) #寻找最短的轴便于生成
    if (shortest_axis == 0):
...
def plot_normal_maps(normal_maps, ground_true, path, epoch, plot_nrow, img_res, indices): #保存法线图以及其gt
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res, indices, exposure=False): #保存rgb图以及其gt
    ground_true = ground_true.cuda()
...
def plot_depth_maps(depth_maps, ground_true, path, epoch, plot_nrow, img_res, indices): #保存深度图以及其gt
    ground_true = ground_true.cuda()
...
def colored_data(x, cmap='jet', d_min=None, d_max=None): #输出彩图数据
    if d_min is None:
        d_min = np.min(x) #归一化
    if d_max is None:
        d_max = np.max(x) #归一化
...
def plot_seg_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res, indices): # 把rgb张量和gt张量做成img并拼接
    ground_true = ground_true.cuda()
    ...
    img = Image.fromarray(tensor)
    img.save('{0}/semantic_{1}_{2}.png'.format(path, epoch, indices[0]))
    

def lin2img(tensor, img_res): #输入线性tensor，给定resolution，返回图像表示的张量（把numsamples拆成res[0]*[1]）
