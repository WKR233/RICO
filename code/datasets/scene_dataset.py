...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        #把所有的图像放到image_paths里面
        image_dir = '{0}/image'.format(self.instance_dir)
        image_paths = sorted(utils.glob_imgs(image_dir))
        self.n_images = len(image_paths)
        #利用cam_file来创建scale_mats和world_mats
        ...
        self.intrinsics_all = []
        self.pose_all = []
        for scale_mat, world_mat in zip(scale_mats, world_mats): #一次取一组对应的scale_mat和world_mat
...
class SceneDatasetDN(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if data_dir == 'syn_data':
            self.instance_dir = os.path.join('../data', data_dir, 'scene{0}'.format(scan_id))
        else:
            self.instance_dir = os.path.join('../data', data_dir, 'scan{0}'.format(scan_id))
