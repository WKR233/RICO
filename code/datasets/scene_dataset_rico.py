...
class RICO_SceneDatasetDN_Mask(torch.utils.data.Dataset):
    ...
    def __len__(self):
        return self.n_images #返回图像数量

    def __getitem__(self, idx): #根据索引得到样本
    ...
    def collate_fn(self, batch_list): #batch_list的元素是getitem返回的样本数据，函数用于组织数据
        # get list of dictionaries and returns input, ground_true as dictionary for all batch instances
        batch_list = zip(*batch_list) #转置
